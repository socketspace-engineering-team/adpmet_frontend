...
$.ajax({
  ...
  success: function (data) {
    ...
    // });
    // Pagination End
    // GET dataDbhLifting API
    for (j = 0; j < dataDbhLifting.length; j++) {
      ...
      $("#itemList").append(`
        ...
        dataDbhLifting[j].provinsi
      }" id="${dataDbhLifting[j].id}">
              <a class="btn card hvr-grow-shadow" href="${`${domain_name}/dbhlifting.html?index=${j}&id=${dataDbhLifting[j].id}`}">
                  <div class="card-image-wrapper">
                      <img src="${
                        dataDbhLifting[j].foto
                      }" class="card-img-top img-fluid" alt="gambar-berita">
                  </div>
                  <div class="card-body">
                      <h6 class="card-title">${dataDbhLifting[j].judul}</h6>
                  </div>
              </a>
          </div>
          `);
