  ...
  $("#provinceList").append(
    `
        <li class="list-group-item px-0 py-1 m-0">
            <a href="#" id="province${i}" class="text-decoration-none province-filter" data-filter='${MOCK_PROVINCE[i].filter_name}'>
...
$.ajax({
  ...
  success: function (data) {
    ...
    // // Pagination End
    // GET dataBumd API
    for (j = 0; j < dataBumd.length; j++) {
      var url = new URL(window.location.href);
      // var itemUrl = dataBumd[j].judul;
      // var itemUrl = itemUrl.replace(/\s+/g, "-");
      $("#itemList").append(`
          <div class="col-md-4 news-container" data-index="${j}" data-filter="${
        dataBumd[j].provinsi
      }" id="${dataBumd[j].id}">
              <a class="btn card hvr-grow-shadow" href="${`${domain_name}/bumdpi.html?index=${j}&id=${dataBumd[j].id}`}">
                  ...
                  <div class="card-body">
                      <h6 class="card-title">${dataBumd[j].judul}</h6>
                  </div>
              </a>
          </div>
          `);
...
provinceFilters.forEach((filter) => {
  filter.addEventListener("click", function () {
    ...
    console.log('Item to Hide',itemsToHide)
  });
});
// Province Filter End
