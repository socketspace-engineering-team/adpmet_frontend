var access_token = localStorage.getItem("access_token");
  ...
  console.log("PREMIUM NEWS READY")
...
//   console.log(newsData.length)
  for (i = 0; i < newsData.length; i++){
    ...
    if(newsData[i].premium !== 0){
      $("#premiumNews").append(
        `<div class="col-md-4 news-container" data-index="${i}" data-filter="${monthNames[newDate.getMonth()]}" id="${newsData[i].id}">
        <a class="btn card hvr-grow-shadow" href="${`https://`+url.hostname+`/beritapremium.html?index=${i}&jdl=${newsUrl}`}">
            <div class="card-image-wrapper">
                <img src="${newsData[i].foto}" class="card-img-top img-fluid" alt="gambar-berita">
            </div>
        <div class="card-body">
